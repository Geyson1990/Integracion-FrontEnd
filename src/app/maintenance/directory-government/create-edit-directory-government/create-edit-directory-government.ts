...
import { DirectoryGovernmentDepartmentDto, DirectoryGovernmentDistrictDto, DirectoryGovernmentDto, DirectoryGovernmentProvinceDto, DirectoryGovernmentSectorDto, DirectoryGovernmentServiceProxy, DirectoryGovernmentTypeDto } from '@shared/service-proxies/application/directory-government-proxie';
import { ModalDirective } from 'ngx-bootstrap/modal';
import { finalize } from 'rxjs/operators';

@Component({
    selector: 'create-edit-directory-government',
    templateUrl: 'create-edit-directory-government.html',
    styleUrls: [
        'create-edit-directory-government.css'
    ]
})
export class CreateEditDirectoryGovernmentComponent extends AppComponentBase {

    @ViewChild('createOrEditModal', { static: true }) modal: ModalDirective;
    @Output() modalSave: EventEmitter<any> = new EventEmitter<any>();

    item: DirectoryGovernmentDto = new DirectoryGovernmentDto();
    ...
    show(id?: number): void {
        ...
        this.saving = false;
        this.item = new DirectoryGovernmentDto();
        this.state = 'true';
        this.departmentId = -1;
        this.provinceId = -1;
        this.districtId = -1;
        ...
        this._directorygovernmentServiceProxy.get(id).subscribe(result => {

            this.departments = result.departments;
            this.sectors = result.sectors;
            this.types = result.types;

            if (result.directoryGovernment) {
                this.item = result.directoryGovernment;
                this.state = result.directoryGovernment.enabled ? 'true' : 'false';
                ...
                this.sectorId = this.item.directoryGovernmentSector.id;

                if (this.sectorId != -1) {
                    const sectorIndex: number = this.sectors.findIndex(p => p.id == this.sectorId);
                    if (sectorIndex == -1) {
                        this.sectors.push(DirectoryGovernmentSectorDto.fromJS(this.item.directoryGovernmentSector));
                        this.sectors = this.sectors.sort((a, b) => a.name.localeCompare(b.name));
                    }
                }

                this.typeId = this.item.directoryGovernmentType.id;

                if (this.typeId != -1) {
                    const typeIndex: number = this.types.findIndex(p => p.id == this.typeId);
                    if (typeIndex == -1) {
                        this.types.push(DirectoryGovernmentTypeDto.fromJS(this.item.directoryGovernmentType));
                        this.types = this.types.sort((a, b) => a.name.localeCompare(b.name));
                    }
                }
            }

            if (this.sectorId == -1)
                this.item.directoryGovernmentSector = undefined;
            if (this.departmentId == -1 || this.provinceId == -1 || this.districtId == -1)
                this.item.district = undefined;
            if (this.typeId == -1)
                this.item.directoryGovernmentType = undefined;
            this.active = true;
            this.modal.show();
        });
    ...
    onDistrictChange(event: any) {
        ...
        if (index != -1) {
            this.item.district = this.districts[index];
        } else {
            this.item.district = undefined;
        }
    }
    ...
    onSectorChange(event: any) {
        ...
        if (index != -1) {
            this.item.directoryGovernmentSector = this.sectors[index];
        } else {
            this.item.directoryGovernmentSector = undefined;
        }
    }
    ...
    onTypeChange(event: any) {
        ...
        if (index != -1) {
            this.item.directoryGovernmentType = this.types[index];
        } else {
            this.item.directoryGovernmentType = undefined;
        }
    }
    ...
    save(): void {
        ...
        if (!this.item.directoryGovernmentType) {
            this.message.info('Debe seleccionar el tipo antes de continuar', 'Aviso');
            return;
        }
        ...
        if (!this.item.directoryGovernmentSector) {
