...
import { PortalSocialConflictRiskDto, PortalSocialConflictGeographycTypeDto, PortalSocialConflictLocationDto, PortalServiceProxy } from '@shared/service-proxies/application/portal-proxie';
...
@Component({
    templateUrl: 'dashboard-social-conflict.component.html',
    styleUrls: [
        '../dashboard.style.css'
    ], animations: [
        appModuleAnimation()
...
export class DashboardSocialConflictComponent extends DashboardComponentBase implements OnInit {
    ...
    ngOnInit(): void {
        this.subscription = this.shared.refresh.subscribe(response => {
            if (response == SharedWindow.All || response == SharedWindow.Social_Conflict)
    ...
    exportImage(getById:string, type:any){
        ...
        if (type == 20) {
            var nameImage = "Alertas_Casos_UnidadTerritorialPie"
        } else if (type == 21) {
            var nameImage = "Alertas_Casos__UnidadTerritorialBar"
        }
        else if (type == 22) {
            var nameImage = "Alertas_Casos_RiesgoPie"
        }
        else if (type == 23) {
            var nameImage = "Alertas_Casos_RiesgoBar"
        }
        else if (type == 24) {
            var nameImage = "Alertas_Casos_GeograficaPie"
        }
        else if (type == 25) {
            var nameImage = "Alertas_Casos_GeograficaBar"
        }
