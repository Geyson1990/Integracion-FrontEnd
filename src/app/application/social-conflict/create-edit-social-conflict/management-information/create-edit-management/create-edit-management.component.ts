...
@Component({
    selector: 'create-edit-management-information-social-conflict',
    templateUrl: 'create-edit-management.component.html',
    styleUrls: [
        'create-edit-management.component.css'
    ]
})
export class CreateEditManagementInformationSocialConflictComponent extends AppComponentBase {
    ...
    item: SocialConflictManagementLocationDto = new SocialConflictManagementLocationDto();
    managementTime: Date;
    active: boolean;
    saving: boolean;
    rowIndex: number;
    ...
    show(rowIndex: number, item: SocialConflictManagementLocationDto, managements: SocialConflictManagementDto[], persons: SocialConflictPersonDto[]): void {
        
        if (item) {
            this.rowIndex = rowIndex;
            this.item = new SocialConflictManagementLocationDto(item);
            this.managementTime = this.item.managementTime?.toDate();
        } else {
            this.rowIndex = undefined;
            this.managementTime = undefined;
            this.item = new SocialConflictManagementLocationDto();
        }
        ...
        if (this.item?.management && this.item.management.id != -1) {
            ...
            if (managementIndex == -1) {
                this.managements.push(SocialConflictManagementDto.fromJS(this.item.management));
