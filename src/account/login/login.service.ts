...
import { AuthenticateModel, AuthenticateResultModel, codeText, ExternalAuthenticateModel, ExternalAuthenticateResultModel, ExternalLoginProviderInfoModel, ResourceCaptchaModelResult, TokenAuthServiceProxy } from '@shared/service-proxies/service-proxies';
...
export class LoginService {
    ...
    generateCaptcha(next: (response: ResourceCaptchaModelResult) => void, error: () => void) {
    ...
    authenticate(finallyCallback?: () => void, redirectUrl?: string, refreshCaptcha?: () => void): void {
        finallyCallback = finallyCallback || (() => {
            this.spinnerService.hide();
        });

        const self = this;
        this._localStorageService.getItem(LoginService.twoFactorRememberClientTokenName, (error: any, value: any) => {
            ...
            self.authenticateModel.userNameOrEmailAddress = codeText(self.authenticateModel.blankUserNameOrEmailAddress);
            self.authenticateModel.password = codeText(self.authenticateModel.blankPassword);
